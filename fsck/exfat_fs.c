...
static int exfat_find_bit(struct exfat *exfat, char *bmap,
			  clus_t start_clu, clus_t *next, bool need_set)
{
	clus_t last_clu;
	...
	while (start_clu < last_clu) {
		if (exfat_bitmap_get(bmap, start_clu) == need_set) {
			*next = start_clu;
			return 0;
		}
		start_clu++;
...
{
	return exfat_find_bit(exfat, bmap, start_clu, next, false);
}

int exfat_find_one_bit(struct exfat *exfat, char *bmap,
		       clus_t start_clu, clus_t *next)
{
	return exfat_find_bit(exfat, bmap, start_clu, next, true);
}
